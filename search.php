...
$offset = ($page - 1) * $limit;
$search = $_GET['search'];
$query = 'SELECT post.post_id , post.title , post.description , post.category , post.post_date , post.post_img , post.author , category.category_id , category.category_name FROM `post` INNER JOIN category ON post.category = category.category_id WHERE post.title
          LIKE "%' . $search . '%" AND post.description LIKE "%' . $search . '%" ORDER BY post.post_id DESC LIMIT ' . $offset . ' , '. $limit . '';
        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div class="post-container">
                  <?php
                  while($fetch_data = $query_data->fetch_assoc()){
                      $admin = null;
                  ...
                  echo '<div class="post-content">
                   ...
                   $qry_total = 'SELECT * FROM `post` WHERE title LIKE "%' . $search . '%" AND description LIKE "%' . $search . '%"';
